...
public interface ContentDao {
   ...
   @Update("update  my_content set title=#{title},channel=#{channel},content=#{content} where id=#{id} ")
    Integer updateArticle(MyContent myContent);

    @Select("select * from my_content where id=#{_param}")
